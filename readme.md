...
Some Linux distributions/\*BSD operating systems offer native packages already. These packages are usually third-party, but we're trying to resolve issues they are facing.
Distro/OS            | Notes
--                   | -----
...
```
cmake -B build # set your standard cmake options, e.g. build type here - For example, -DCMAKE_BUILD_TYPE=RelWithDebInfo
cd build
